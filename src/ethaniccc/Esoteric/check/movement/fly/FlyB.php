...
class FlyB extends Check{
    ...
    public function inbound(DataPacket $packet, PlayerData $data): void{
        if($packet instanceof MovePlayerPacket && $data->offGroundTicks >= 10 && $data->ticksSinceFlight >= 10){
            ...
            if($difference <= 4E-5 && $data->currentMoveDelta->y > -3.0 && $data->ticksSinceInCobweb >= 10 && !$data->teleported && $data->ticksSinceInClimbable >= 10 && $data->ticksSinceInLiquid >= 10
            && $data->ticksSinceMotion >= 5){
                $this->flag($data);
                $this->setback($data);
            }
        }
