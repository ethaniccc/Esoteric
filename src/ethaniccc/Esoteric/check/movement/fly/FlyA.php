...
class FlyA extends Check{
    ...
    public function inbound(DataPacket $packet, PlayerData $data): void{
        if($packet instanceof MovePlayerPacket && $data->offGroundTicks >= 3 && $data->ticksSinceFlight >= 10){
            ...
            if($difference > $this->option("diff_max", 0.015) && !$data->teleported && $data->ticksSinceMotion > 1 && $data->ticksSinceInLiquid >= 5 && $data->ticksSinceInClimbable >= 5
            && $data->ticksSinceInCobweb >= 5 && abs($predictedYMovement) > 0.005 && !$data->isCollidedHorizontally){
                if(++$this->buffer >= 2){
                    $this->flag($data, [
