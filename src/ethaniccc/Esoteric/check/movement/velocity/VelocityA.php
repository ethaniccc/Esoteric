    ...
    public function inbound(DataPacket $packet, PlayerData $data): void{
        if($packet instanceof MovePlayerPacket){
            ...
            if($this->yMotion > 0.005){
                ...
                if($diff < -4.26E-7 && !$data->teleported && !$data->hasBlockAbove && $data->ticksSinceInCobweb >= 5 && $data->ticksSinceFlight >= 10 && $data->ticksSinceInLiquid >= 5
                && $data->ticksSinceInClimbable >= 5){
                    if(++$this->buffer > 4){
                        $this->flag($data, [
                            "pct" => round($percentage, 5) . "%",
                        ]);
                    }
