...
class VelocityB extends Check {
	...
	public function inbound(ServerboundPacket $packet, PlayerData $data): void {
		if($packet instanceof PlayerAuthInputPacket && $data->ticksSinceMotion === 1 && $data->offGroundTicks > 2 && $data->ticksSinceTeleport >= 3 && $data->ticksSinceSpawn >= 20){
			...
			$keys = implode(',', $keys);
			$subVec = $data->currentMoveDelta->subtractVector($expectedMovement);
			if (!$data->isCollidedHorizontally && $data->ticksSinceInClimbable > 5 && $data->ticksSinceFlight > 5 && !$data->isGliding && $data->ticksSinceInCobweb > 5 && $data->ticksSinceInLiquid > 5) {
				if(abs($subVec->x) > 5E-5 && abs($subVec->z) > 5E-5 && $pct < 100) {
					$r = round($pct, 3);
