...
class MotionB extends Check{
    ...
    public function inbound(DataPacket $packet, PlayerData $data): void{
        if($packet instanceof MovePlayerPacket && $data->offGroundTicks >= 3 && $data->ticksSinceFlight >= 10){
            ...
            if($diff > 0.00001 && $data->ticksSinceMotion > 1 && $data->ticksSinceInCobweb >= 10 && $data->ticksSinceInClimbable >= 10
                && $data->ticksSinceInLiquid >= 10 && $currentXZ > 0 && $lastXZ > 0 && !$data->teleported){
                if(++$this->buffer >= 2){
                    $this->flag($data, [
                        "diff" => round($diff, 5),
                    ]);
