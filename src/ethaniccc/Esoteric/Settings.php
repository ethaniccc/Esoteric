...
final class Settings{
	...
	public function getSetbackType() : string{
		if($this->setbackType === null){
			$this->setbackType = match(true){
				isset($this->data["setback_type"]) => match(true){
					in_array($this->data["setback_type"], [self::SETBACK_INSTANT, self::SETBACK_SMOOTH], true) => $this->data["setback_type"],
					default => "none"
				},
				default => "none"
			};
